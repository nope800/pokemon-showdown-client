...
(function (exports, $) {
	...
	Search.prototype.renderPokemonSortRow = function () {
		...
		if (this.engine.dex.gen >= 2) {
			buf += '<button class="sortcol statsortcol' + (this.sortCol === 'boa' ? ' cur' : '') + '" data-sort="boa">BoA</button>';
			buf += '<button class="sortcol statsortcol' + (this.sortCol === 'bod' ? ' cur' : '') + '" data-sort="bod">BoD</button>';
		} else {
			buf += '<button class="sortcol statsortcol' + (this.sortCol === 'boa' ? ' cur' : '') + '" data-sort="boa">Spc</button>';
		}
	...
	Search.prototype.renderPokemonRow = function (pokemon, matchStart, matchLength, errorMessage, attrs) {
		...
		if (gen >= 2) {
			buf += '<span class="col statcol"><em>BoA</em><br />' + stats.boa + '</span> ';
			buf += '<span class="col statcol"><em>BoD</em><br />' + stats.bod + '</span> ';
		} else {
			buf += '<span class="col statcol"><em>Spc</em><br />' + stats.boa + '</span> ';
		}
		buf += '<span class="col statcol"><em>Hor</em><br />' + stats.hor + '</span> ';
		var bst = 0;
		for (i in stats) {
			if (i === 'bod' && gen === 1) continue;
			bst += stats[i];
		}
	...
	Search.prototype.renderTaggedPokemonRowInner = function (pokemon, tag, errorMessage) {
		...
		buf += '<span class="col statcol"><em>BoD</em><br />' + pokemon.baseStats.bod + '</span> ';
	...
	Search.prototype.renderMoveRow = function (move, matchStart, matchLength, errorMessage, attrs) {
		...
		// power, accuracy, pp
		var pp = (move.pp === 1 || move.noPPBoosts ? move.pp : move.pp); //amoros: pp is pp
	...
	Search.prototype.renderMoveRowInner = function (move, errorMessage) {
		...
		// power, accuracy, pp
		var pp = (move.pp === 1 || move.noPPBoosts ? move.pp : move.pp); //pp is pp amoros
		if (this.engine && this.engine.dex.gen < 3) pp = Math.min(61, pp);
		buf += '<span class="col labelcol">' + (move.category !== 'Status' ? ('<em>Power</em><br />' + (move.basePower || '&mdash;')) : '') + '</span> ';
		buf += '<span class="col widelabelcol"><em>Accuracy</em><br />' + (move.accuracy && move.accuracy !== true ? move.accuracy + '%' : '&mdash;') + '</span> ';
		buf += '<span class="col pplabelcol"><em>PP</em><br />' + pp + '</span> ';
	...
	Search.prototype.renderTaggedMoveRow = function (move, tag, errorMessage) {
		...
		buf += '<span class="col labelcol">' + (move.category !== 'Status' ? ('<em>Power</em><br />' + (move.basePower || '&mdash;')) : '') + '</span> ';
		buf += '<span class="col widelabelcol"><em>Accuracy</em><br />' + (move.accuracy && move.accuracy !== true ? move.accuracy + '%' : '&mdash;') + '</span> ';
		buf += '<span class="col pplabelcol"><em>PP</em><br />' + (move.pp !== 1 ? move.pp : move.pp) + '</span> ';
		//amoros: pp is pp
